...
export const checkboxSchema=z.object({label:z.string(),section:z.enum(['REGCARD_DETAILS','TERMS_AND_CONDITIONS','UNCERTAIN']),mappedField:z.string().optional(),confidence:z.number().min(0).max(1),x:z.number().min(0).max(1).optional(),y:z.number().min(0).max(1).optional()});
