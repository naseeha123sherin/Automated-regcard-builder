...
export const projectSchema = z.object({ version: z.literal(1), id: z.string(), projectName: z.string(), propertyName: z.string(), propertyNameConfidence: z.number(), propertyReviewed: z.boolean(), orientation: z.enum(['portrait', 'landscape']), sourceDocument: z.object({ name: z.string(), mime: z.string() }).nullable(), logo: z.object({ dataUrl: z.string().regex(/^data:image\/(png|jpeg);base64,[A-Za-z0-9+/]+={0,2}$/), mime: z.enum(['image/png', 'image/jpeg']) }).nullable(), elements: z.array(elementSchema), fieldMappings: z.array(z.object({ id: z.string(), detectedLabel: z.string(), fieldName: z.string().nullable(), fieldLabel: z.string(), confidence: z.number(), status: z.enum(['confirmed', 'suggested', 'requires_review', 'ignored']) })), regcardConfig: configSchema, checkinTermsConfig: configSchema, checkoutTermsConfig: configSchema, termsReviewed: z.boolean(), analysisWarnings: z.array(z.string()), rdlConfig: z.object({ pageWidth: z.literal(19), pageHeight: z.literal(24) }), previewSampleData: z.record(z.string()), previewScreen: z.enum(['registration', 'terms', 'signature', 'checkout']) });
