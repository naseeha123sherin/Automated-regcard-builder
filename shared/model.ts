...
export const checkboxSchema=z.object({label:z.string(),groupLabel:z.string().optional(),section:z.enum(['REGCARD_DETAILS','TERMS_AND_CONDITIONS','UNCERTAIN']),mappedField:z.string().optional(),confidence:z.number().min(0).max(1),x:z.number().min(0).max(1).optional(),y:z.number().min(0).max(1).optional()});
export const analysisSchema = z.object({ propertyName: z.string(), propertyNameConfidence: z.number().min(0).max(1), orientation: z.enum(['portrait', 'landscape']), elements: z.array(elementSchema).max(500), detectedLabels: z.array(z.string()).max(150), terms: z.array(z.string()).max(100), checkboxLabels: z.array(z.string()).default([]), checkboxes:z.array(checkboxSchema).default([]), warnings: z.array(z.string()).default([]) });
...
export const projectSchema = z.object({ version: z.literal(1), id: z.string(), systemMode:z.enum(['mirror','kiosk']).default('mirror'), propertyName: z.string(), propertyNameConfidence: z.number(), propertyReviewed: z.boolean(), orientation: z.enum(['portrait', 'landscape']), sourceDocument: z.object({ name: z.string(), mime: z.string() }).nullable(), sourceRdl: z.object({ name: z.string() }).nullable(), discoveredRdlFields: z.array(z.string()), customFields: z.array(z.string()), kioskCheckboxImages:z.object({checked:z.string(),empty:z.string()}).nullable().default(null), classifiedCheckboxes:z.array(checkboxSchema.extend({id:z.string()})), logo: z.object({ dataUrl: z.string().regex(/^data:image\/(png|jpeg);base64,[A-Za-z0-9+/]+={0,2}$/), mime: z.enum(['image/png', 'image/jpeg']) }).nullable(), elements: z.array(elementSchema), fieldMappings: z.array(z.object({ id: z.string(), detectedLabel: z.string(), detectedValue: z.string().optional(), elementId: z.string().optional(), expression: z.string().optional(), fieldName: z.string().nullable(), fieldLabel: z.string(), confidence: z.number(), status: z.enum(['confirmed', 'suggested', 'requires_review', 'ignored']) })), regcardConfig: configSchema, checkinTermsConfig: configSchema, checkoutTermsConfig: configSchema, termsReviewed: z.boolean(), visualReviewed: z.boolean(), analysisWarnings: z.array(z.string()), rdlConfig: z.object({ pageWidth: z.number().positive(), pageHeight: z.number().positive() }), previewSampleData: z.record(z.string()), previewScreen: z.enum(['registration', 'terms', 'signature', 'checkout']) });
