...
export function rdlReadiness(p:Project):Issue[]{const out:Issue[]=[];if(!p.visualReviewed)out.push({path:'RDL visual review',message:'Visual comparison has not been completed.',severity:'warning'});if(p.elements.some(e=>e.kind==='image'&&!e.imageReviewed))out.push({path:'Document images',message:'Unreviewed image areas are omitted from the RDL.',severity:'warning'});for(const m of p.fieldMappings)if(!m.fieldName)out.push({path:m.detectedLabel,message:'Unmapped field is included without a Lookup expression.',severity:'warning'});else if(m.confidence<.9)out.push({path:m.detectedLabel,message:'Low-confidence automatic mapping may require editing.',severity:'warning'});return out;}
