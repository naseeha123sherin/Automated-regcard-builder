...
export function validateConfig(value: unknown, kind: 'regcard' | 'checkin' | 'checkout', reference?: Config): Issue[] { const r = configSchema.safeParse(value); if (!r.success)
    ...
    for (const key of Object.keys(ref))
        if (!(key in f))
            issues.push({ path: path + '.' + key, message: 'Required reference property was removed.', severity: 'error' }); if (!['input-text', 'text', 'line-break', 'line-separator', 'checkbox'].includes(f.field_type))
...
export function readiness(p: Project): Issue[] { const out: Issue[] = []; if (!p.propertyReviewed || !p.propertyName.trim())
    out.push({ path: 'propertyName', message: 'Confirm the current property name.', severity: 'error' }); if (!p.termsReviewed)
    out.push({ path: 'terms', message: 'Review and confirm source terms before downloading.', severity: 'error' }); for (const m of p.fieldMappings)
    if (!['confirmed', 'ignored'].includes(m.status))
        out.push({ path: m.detectedLabel, message: 'Accept or ignore this mapping.', severity: 'error' }); const checkoutProperty = fields(p.checkoutTermsConfig).filter(f => f.field_type === 'text')[1]?.field_value; if (checkoutProperty !== p.propertyName)
    out.push({ path: 'checkout property', message: 'Checkout property differs from project name. Correct it in Project or JSON.', severity: 'error' }); return out; }
