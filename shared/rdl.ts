...
const buildReportItems=(items:string[])=>{const valid=items.filter(x=>/^<(?:Textbox|Line|Rectangle|Image)\b/.test(x));return valid.length?`<ReportItems>${valid.join('')}</ReportItems>`:'';};
export function generateRdl(p:Project):string {
 ...
 const items=es.map((e,i)=>{
  ...
  if(e.kind==='rectangle')return {layer:0,xml:`<Rectangle Name="Rectangle_${i}">${pos}<Style>${border}<BackgroundColor>${e.background||'Transparent'}</BackgroundColor></Style></Rectangle>`};
  if(e.kind==='image'){
 ...
 }).sort((a,b)=>a.layer-b.layer).map(x=>x.xml).filter(Boolean);
 let logo='';
