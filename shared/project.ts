...
export function newProject(t: Templates): Project {
 ...
 return {version:1,id:crypto.randomUUID(),propertyName:'',propertyNameConfidence:0,propertyReviewed:false,orientation:'landscape',sourceDocument:null,sourceRdl:null,discoveredRdlFields:[],customFields:[],logo:null,elements:[],fieldMappings:[],regcardConfig:clone(t.regcard),checkinTermsConfig:checkin,checkoutTermsConfig:checkout,termsReviewed:false,visualReviewed:false,analysisWarnings:[],rdlConfig:{pageWidth:20,pageHeight:25},previewSampleData:clone(sampleData),previewScreen:'registration'};
}
...
export function applyAnalysis(p:Project,a:Analysis,t:Templates):Project {
 ...
 q.fieldMappings=q.elements.filter(e=>e.kind==='dynamic').map((e,i)=>{
  ...
  m.expression=m.fieldName?lookup(m.fieldName):'';return m;
 });
 ...
 q.checkinTermsConfig=currentTerms(t,a.terms.join('\n\n'),a.checkboxLabels);q.termsReviewed=false;q.analysisWarnings=a.warnings;return q;
}
...
export function acceptMapping(p:Project,id:string):Project {
 ...
 const e=q.elements.find(e=>e.id===m.elementId);if(e)e.fieldName=m.fieldName;return q;
}
export function addMissingField(p:Project,name:string,label:string,bounds:Pick<Element,'x'|'y'|'width'|'height'>):Project {
 ...
 q.elements.push({id,kind:'dynamic',fieldName:name,detectedLabel:label,...bounds,fontSize:9,bold:false,align:'Left'});
