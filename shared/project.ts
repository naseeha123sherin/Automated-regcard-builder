...
const canonicalCheckboxes=<T extends {label:string;groupLabel?:string;section:string;x?:number;y?:number}>(items:T[])=>items.filter((c,i,list)=>list.findIndex(o=>checkboxKey(o)===checkboxKey(c)&&((o.x===undefined||c.x===undefined)||Math.abs(o.x-c.x)<.03)&&((o.y===undefined||c.y===undefined)||Math.abs(o.y-c.y)<.03))===i);
