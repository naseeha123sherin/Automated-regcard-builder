...
export default function App() {
 ...
 {tab === 'Field Mapping' && <><div className="metrics">{[[project.fieldMappings.length, 'Dynamic fields'], [project.fieldMappings.filter(m => !!m.fieldName).length, 'Auto-mapped'], [project.fieldMappings.filter(m => !m.fieldName).length, 'May need review'], [fields(project.regcardConfig).filter(f => f.field_name).length, 'Mirror fields']].map(([n, label]) => <div key={label}><strong>{n}</strong><span>{label}</span></div>)}</div><div className="panel"><h2>Manual document understanding</h2><div className="grid"><label className="form-label">Current property<input placeholder="From current document" value={project.propertyName} onChange={e => setProject(setProperty(project, e.target.value))}/></label><div><label className="form-label">Source labels · one per line<textarea rows={4} value={labels} onChange={e => setLabels(e.target.value)} placeholder={'Guest Name / اسم الضيف\nRoom No.\nFlight Number'}/></label><button onClick={() => { if (!labels.trim()) {
