...
import './upload.css';
import { FileStack, Upload, ArrowRight, ShieldCheck, Download, FileCode2, Tablet, SlidersHorizontal, Braces, Files, Plus } from 'lucide-react';
import { ReferenceTemplateService } from '../shared/templates';
import { applyAnalysis, newProject, setProperty, setTerms } from '../shared/project';
...
export default function App() {
    const [project,setProject]=useState<Project>(()=>newProject(templates)),[tab,setTab]=useState('Upload & Analyze'),[jsonTab,setJsonTab]=useState<'regcard'|'checkin'|'checkout'>('regcard'),[source,setSource]=useState<{dataUrl:string;preview:string;text:string;pages:number;analysis:Analysis|null}|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState(''),[labels,setLabels]=useState(''),[vision,setVision]=useState(false),[selectedElement,setSelectedElement]=useState<string|null>(null),[status,setStatus]=useState({provider:'none',configured:false});
    ...
    function reset() { if (!confirm('Start a new session? Unsaved work will be discarded.'))
        return; setProject(newProject(templates)); setSource(null); setLabels(''); setTab('Upload & Analyze'); setNotice('New session started.'); }
    ...
    return <div className="app"><aside><div className="brand"><FileStack /> REG CARD<br />AUTOMATOR</div><span className="eyebrow">ENGINEERING WORKSPACE</span><nav>{navigation.map(([name, Icon]) => <button className={tab === name ? 'active' : ''} onClick={() => setTab(name)} key={name}><Icon size={17}/>{name}</button>)}</nav><div className="aside-bottom"><ShieldCheck size={18}/><span>Source documents stay in memory.<br />You control the final output.</span></div></aside><main><header><div>{project.propertyName||'Property pending'} <span>/</span> {project.sourceDocument?.name||'No PDF uploaded'}</div><div className="toolbar compact"><span className="pill">{project.elements.length?'Analyzed · needs review':'Awaiting PDF'}</span><span className="pill">Memory only</span></div></header><section className="page"><div className="title"><div><p className="eyebrow">{String(navigation.findIndex(([n]) => n === tab) + 1).padStart(2, '0')} / {tab.toUpperCase()}</p><h1>{titles[tab][0]}</h1><p>{titles[tab][1]}</p></div><button onClick={reset}><Plus size={16}/> New session</button></div>{error && <div className="error" role="alert">{error}<button onClick={() => setError('')}>Dismiss</button></div>}{notice && <div className="notice" role="status">{notice}</div>}
